...
def CalcCurvature(FV, VertexNormals, FaceNormals, Avertex, Acorner, up, vp):
	...
	for i in range(FV.triangles.shape[0]):
	...
	wfp = np.array(np.zeros((FV.triangles.shape[0], 3)))
	for i in range(FV.triangles.shape[0]):
		...
		n0 = VertexNormals[FV.triangles[i][0], :]
		n1 = VertexNormals[FV.triangles[i][1], :]
		n2 = VertexNormals[FV.triangles[i][2], :]
		...
		"""
		wfp[i][0] = Acorner[i][0] / Avertex[FV.triangles[i][0]]
		wfp[i][1] = Acorner[i][1] / Avertex[FV.triangles[i][1]]
		wfp[i][2] = Acorner[i][2] / Avertex[FV.triangles[i][2]]
		...
		for j in range(3):
			new_ku, new_kuv, new_kv = ProjectCurvatureTensor(t, B, nf, x[0][0], x[0][1], x[0][2],
			                                                 up[FV.triangles[i][j], :], vp[FV.triangles[i][j], :])
			VertexSFM[FV.triangles[i][j]] += np.dot(wfp[i][j], np.array([[new_ku, new_kuv], [new_kuv, new_kv]]))
...
def CalcFaceNormals(FV):
	...
	#notation: triangles was changed from faces  
	e0 = FV.vertices[FV.triangles[:, 2], :] - FV.vertices[FV.triangles[:, 1], :]
	e1 = FV.vertices[FV.triangles[:, 0], :] - FV.vertices[FV.triangles[:, 2], :]
...
def CalcVertexNormals(FV, N):
	...
	"Get all the edge vectors"
	e0 = np.array(FV.vertices[FV.triangles[:, 2], :] - FV.vertices[FV.triangles[:, 1], :])
	e1 = np.array(FV.vertices[FV.triangles[:, 0], :] - FV.vertices[FV.triangles[:, 2], :])
	e2 = np.array(FV.vertices[FV.triangles[:, 1], :] - FV.vertices[FV.triangles[:, 0], :])
	...
	"Calc weights"
	Acorner = np.zeros((np.shape(FV.triangles)[0], 3))
	...
	for i in range(np.shape(FV.triangles)[0]):
		wfv1 = Af[i] / ((de1[i] ** 2) * (de2[i] ** 2))
		wfv2 = Af[i] / ((de0[i] ** 2) * (de2[i] ** 2))
		wfv3 = Af[i] / ((de1[i] ** 2) * (de0[i] ** 2))
		
		VertexNormals[FV.triangles[i][0], :] += wfv1 * N[i, :]
		VertexNormals[FV.triangles[i][1], :] += wfv2 * N[i, :]
		VertexNormals[FV.triangles[i][2], :] += wfv3 * N[i, :]
		...
		Avertex[FV.triangles[i][0]] += Acorner[i][0]
		Avertex[FV.triangles[i][1]] += Acorner[i][1]
		Avertex[FV.triangles[i][2]] += Acorner[i][2]
		
		" Calcul initial coordinate system "
		up[FV.triangles[i][0], :] = e2_norm[i, :]
		up[FV.triangles[i][1], :] = e0_norm[i, :]
		up[FV.triangles[i][2], :] = e1_norm[i, :]
